...
kdb = kdb.dropna(subset=["Age", "FeH"])

# fit linear regression
x = kdb["FeH"].values.reshape(-1, 1)
y = kdb["Age"].values
model = LinearRegression().fit(x, y)
y_pred = model.predict(x)

# Compute residuals
kdb["Residual"] = y - y_pred

# Calculate IQR
q1 = kdb["Residual"].quantile(0.25)
q3 = kdb["Residual"].quantile(0.75)
iqr = q3 - q1
...
# Flag outliers
outliers = kdb[(kdb["Residual"] < lower_bound) | (kdb["Residual"] > upper_bound)]
inliers  = kdb[(kdb["Residual"] >= lower_bound) & (kdb["Residual"] <= upper_bound)]
...
plt.plot(kdb["FeH"], y_pred, c="black", label="Fit")

# Labels for outliers
for _, row in outliers.iterrows():
    plt.text(row["FeH"], row["Age"]+0.2, row["Object"], fontsize=8, ha="center")

plt.xlabel("Metallicity [Fe/H]")
plt.ylabel("Age (Gyr)")
plt.title("Age–Metallicity Relation with Outliers")
plt.legend()
plt.savefig("results/age_metallicity_krause.png")
plt.show()